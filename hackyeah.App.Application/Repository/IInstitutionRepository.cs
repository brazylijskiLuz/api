...
public interface IInstitutionRepository<T> : IBaseRepository<T> where T : Entity
{
    ...
    Task<List<T>> GetByQueryAsync(string query, int page, List<InstitutionType> institutionTypes, int requestMinPrice, int requestMaxPrice,int pageSize, ModeOfStudy mode,
